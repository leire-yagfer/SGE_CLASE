...
class Animal:
    ...
    def sound(self): # las clases que heredan de Animal pueden sobrescribir este método para hacer un sonido específico
        pass #cuando se necesita que un bloque de código esté presente, pero no se desea que haga nada en ese momento
...
my_animal.sound() # no hace nada pq en el método no tiene código
my_dog = Dog("Perro") ##llamo a la clase Dog y le paso el nombre nombre del animal perro (Animal.name = Perro)
my_dog.sound()
my_cat = Cat("Gato")
my_cat.sound()
